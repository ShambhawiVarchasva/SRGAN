...
Flags.DEFINE_string('perceptual_mode', 'V  GG54', 'The type of feature used in perceptual loss')
...
# the inference mode (just perform super resolution on the input image)
    ...
    with tf.variable_scope('generator'):
        if FLAGS.task == 'SRGAN' or FLAGS.task == 'SRResnet':
            print("Gbm ",inputs_raw.get_shape().as_list())
            print("Start  ")
            gen_output = generator(inputs_raw, 3, reuse=False, FLAGS=FLAGS)
            print("End  ")
            print("Gen  Gbm ",gen_output.get_shape().as_list())
        else:
            raise NotImplementedError('Unknown task!!')
    ...
    with tf.Session(config=config) as sess:
        ...
        for i in range(max_iter):
            ...
            print("evals",inference_data.inputs[i].shape)
...
# The training mode
    ...
    if FLAGS.task == 'SRGAN':
        print(" data.inputs, data.targets ",data.inputs, data.targets)
    ...
    with sv.managed_session(config=config) as sess:
        if (FLAGS.checkpoint is not None) and (FLAGS.pre_trained_model is False):
            print('Loading model from the checkpoint.. hhhhh.')
            checkpoint = tf.train.latest_checkpoint(FLAGS.checkpoint)
            print(FLAGS.checkpoint)
            saver.restore(sess, "./experiment_SRGAN_MSE/model-500000")
