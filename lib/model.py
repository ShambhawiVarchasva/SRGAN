...
# Definition of the  my_generator
...
# Check the flag
    ...
    with tf.variable_scope('generator_unit', reuse=reuse):
        ...
        print("upsampled_2_gen_inputs ",upsampled_2_gen_inputs )
...
# Definition of the generator
def generator_real(gen_inputs, gen_output_channels, reuse=False, FLAGS=None):
    # Check the flag
    print("gen_inputs ",gen_inputs)
    if FLAGS is None:
        raise  ValueError('No FLAGS is provided for generator')
...
def save_images(fetches, FLAGS, step=None):
    ...
    if FLAGS.mode == 'inference':
        ...
        for kind in ["inputs", "outputs", "targets"]:
            ...
            with open(out_path, "wb") as f:
                f.write(contents)
            img.append(cv2.imread(out_path))
            
      
        filesets.append(fileset)
        if(img[1].shape!=img[2].shape):
         img[1] = cv2.resize(img[1], (img[2].shape[1],img[2].shape[0]),interpolation = cv2.INTER_NEAREST)
        psnr=(calculate_psnr(img[1], img[2]))
        ssim=(calculate_ssim(img[1], img[2]))

    return filesets,psnr,ssim
